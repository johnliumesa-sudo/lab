...
******** getNearbyNPCs
#+begin_src lua
  local function getNearbyNPCs(swordRange)
     while true do
	local swordHandle = script.Parent:FindFirstChild("Handle")
	if not swordHandle then return {} end
	local swordPosition = swordHandle.Position
	local nearbyNPCs = {}
	-- Use GetPartBoundsInRadius for efficient, non-collision based spatial query
	local partsInBounds = Workspace:GetPartBoundsInRadius(swordPosition, swordRange)
	for _, part in ipairs(partsInBounds) do
	   local model = part.Parent
	   -- Check if the part belongs to an NPC model
	   if model and model:IsA("Model") and model.Parent and model.Parent == NPCsFolder then
	if part.Name == "HumanoidRootPart" and model:FindFirstChildWhichIsA("Humanoid") then
	   table.insert(nearbyNPCs, model)
	   print(model.Name)
	end
	   end
	end
...
#+begin_src lua
  if script.Parent.Parent == swordFolder then
    script.Parent.Equipped:Connect(onEquipped)
    script.Parent.Equipped:Connect(getNearbyNPCs)
  end  
#+end_src
**** tool spawner
***** ToolSpawnerScript
